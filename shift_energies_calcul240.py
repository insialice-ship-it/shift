...
def calcul_shift(nom_fichier):
	donnees = lire_fichier_donnees(nom_fichier)

	milieu = (donnees[4][0]-donnees[3][0])/2 + donnees[3][0]
	ev_calc = nm_to_eV(milieu)
	print (milieu)
	print (ev_calc)
	print (nm_to_eV(240))
	shift = nm_to_eV(240) - ev_calc
	return shift
...
shift = calcul_shift("cdspectrum")
print (f"On calcule un shift de {shift} eV")
donnees_decalees=decalage("cdspectrum", shift)
np.savetxt("cdspectrum_shifted_calcul240", donnees_decalees, delimiter="  ")
